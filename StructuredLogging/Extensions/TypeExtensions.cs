...
namespace StructuredLogging.Extensions
{
    static class TypeExtensions
    {
        public static IEnumerable<PropertyInfo> GetPropertiesRecursive(this Type type)
        {
            ...
            {
                var unseenProperties = currentTypeInfo.DeclaredProperties.Where(p => p.CanRead && p.GetMethod.IsPublic && !p.GetMethod.IsStatic && (p.Name != "Item" || p.GetIndexParameters().Length == 0) && !seenNames.Contains(p.Name));
